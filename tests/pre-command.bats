...
# way clap rejects an unknown subcommand, so the hook has to try the next name
# rather than mistaking a no-op for success.
stub_aspect() {
  local group="${1:-setup}"
...
@test "passes the configured remote, home and force through to the rc task" {
  stub_cloud_aspect
  stub_bazel
  export BUILDKITE_PLUGIN_SETUP_ASPECT_BAZELRC_REMOTE="exec"
  export BUILDKITE_PLUGIN_SETUP_ASPECT_BAZELRC_FORCE="true"
...
@test "passes a configured home straight through" {
  stub_cloud_aspect
  stub_bazel
  export BUILDKITE_PLUGIN_SETUP_ASPECT_BAZELRC_HOME="false"
...
@test "passes no rc flags when the plugin is unconfigured" {
  stub_cloud_aspect
  stub_bazel
...
@test "writes no rc when generation is turned off" {
  stub_cloud_aspect
  stub_bazel
  export BUILDKITE_PLUGIN_SETUP_ASPECT_BAZELRC_GENERATE="false"
...
@test "prefers \`aspect setup bazelrc\` to generate ~/.bazelrc" {
  ...
  # Header-flag values are redacted in the echoed rc: the flag and header name
  # stay visible but the x-identity value is masked.
...
@test "falls back to \`rosetta bazelrc\` when aspect is too old, with an upgrade hint" {
  ...
  assert_success
  # Neither name worked, so the hook points users at the aspect-cli releases.
